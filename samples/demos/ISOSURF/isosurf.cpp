...
#include "readtex.ch"   /* I know, this is a hack.  KW: me too. */
...
#define IPOLYGON_FILL   0x10000000
...
#define POLYGON_MASK           (IPOLYGON_FILL|POLYGON_LINE)
...
#if defined(__IBMCPP__) ||  defined(__IBMCPP__)
#define QSORT_COMPAPI _Optlink
#else
#define QSORT_COMPAPI
#endif

#define COMPARE_FUNC( AXIS )                            \
static int QSORT_COMPAPI compare_axis_##AXIS( const void *a, const void *b ) \
{                                                      \
...
int (QSORT_COMPAPI  * (compare[7]))( const void *a, const void *b ) =
{
   compare_axis_0,
   compare_axis_1,
   compare_axis_2,
...
static void GLUTCALLBACK ModeMenu(int m)
{
   m &= allowed;
...
static void  GLUTCALLBACK SpecialKey( int key, int x, int y )
{
   (void) x;
   (void) y;
   switch (key) {
...
maininit(int argc, char **argv)
{
   ...
   glutAddMenuEntry("", 0);
   glutAddMenuEntry("Polygon Mode Fill",     IPOLYGON_FILL);
