...
static void GLUTCALLBACK Key2(int key, int x, int y)
{
    (void) x;
    (void) y;
    switch (key) {
...
static GLenum Args(int argc, char **argv)
{
    ...
    for (i = 1; i < argc; i++) {
       if (strcmp(argv[i], "-ci") == 0) {
	    ...
	    printf("Usage: logo1.exe [-ci|-rgb] [-sb|-db] [-?]\n");
       } else {
           printf("%s (Bad option).\n", argv[i]);
           return GL_FALSE;
       }
...
int maininit(int argc, char** argv)
{
    ...
    glutInit(&argc, argv);
#if USE_MESA
   printf("Press 1,2,3,4,5,6,8,9,0 Zz,pqwertyuasdfgnl, Left,Right,Up,Down  or Esc\n"); fflush(stdout);
#endif
