...
static void  GLUTCALLBACK special(int k, int x, int y)
{
  switch(k) {
  case GLUT_KEY_LEFT:
    alpha-=2.0;
...
static void GLUTCALLBACK  reshape(int w, int h)
{
  WIDTH=w;
  HEIGHT=h;
...
static void  GLUTCALLBACK draw(void)
{
  ...
  if((count % FRAME)==0) {
    fr=gettime();
    FrameRate = FRAME/fr;
    sprintf(frbuf,"Frame rate: %f \r",FrameRate);
  }
...
{
  float lamb[4]={0.5,0.4,0.3,0.8};
  float lspec[4]={1.0,0.8,1.0,1.0};
...
#if USE_MESA
    printf("Press azjhftb,  Up,Down,Left,Right, or Esc\n"); fflush(stdout);
#endif
